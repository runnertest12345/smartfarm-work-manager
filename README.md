...
- 통합 현황·프로젝트 관리 상단의 연도·사업 타입별 사업·업무·설치·구독·서류·정산 KPI
- 연도별 전체 사업과 프로젝트 진행 단계·막힘 현황
- 프로젝트별 하위 업무, 설치·시운전·교육 진행률
- 필수 제출서류, 정산 상태·금액·증빙 관리
- 전체/구독/미구독, 작목별, 지역별 농가 현황
- 실제 입금 이력 기반 최초 구독, 현재 1차·2차·3차 이상 연장 현황
- 오늘 기준 만료 후 결과 미등록·만료 예정과 갱신·이탈·재가입 실적
...
[화면·카드 문서 전체 목차](docs/ui/README.md)에서 각 메뉴, 상세 화면, 카드, 등록·수정 입력창의 설명을 확인할 수 있습니다. 표시 정보, 계산식, 필터 적용 범위, 버튼 동작과 연결 데이터를 현재 구현 기준으로 정리했습니다.

- [공통 화면 이동](docs/ui/common/navigation.md)
- [데이터 연결과 집계 단위](docs/ui/common/data-model.md)
- [구독률·갱신률 등 계산 기준](docs/ui/common/calculation-rules.md)
...
KPI 계산 검증은 실제 데이터를 변경하지 않는 고정 예제로 실행합니다.

```bash
node scripts/test-dashboard-kpis.mjs
```
