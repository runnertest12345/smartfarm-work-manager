          ...
          >
            ...
            <div className="space-y-3">
              {cards.map((group) => {
                ...
                const autoExpanded =
                  searching && children.some((row) => row.matched);
                const isExpanded = expanded.has(item.id) || autoExpanded;
                ...
                return (
                  <article
                    ...
                    className={`space-y-3 rounded-xl border border-slate-200 bg-white p-4 ${savingId && rows.some((row) => row.item.id === savingId) ? 'opacity-60' : ''}`}
                  >
                    <div className="flex items-start gap-2">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <button
                          ...
                          disabled={busy}
                          onClick={() => onOpen(item)}
                          className="mt-1 min-h-9 w-full break-words text-left text-base font-semibold hover:text-emerald-800 hover:underline"
                        >
                          {item.title}
                        </button>
                      </div>
                      ...
                      <>
                        ...
                        >
                          {FARM_WORK_STATUSES.map((key) => (
                            ...
                            >
                              {FARM_WORK_STATUS_LABELS[key]} {group.counts[key]}
                            </span>
                          ))}
                        </div>
                    ...
                    {group.needsConfirmation && (
                      <div className="space-y-2 rounded-lg bg-emerald-50 p-3 text-sm text-emerald-950">
                        <p>
                          {group.readyToConfirm
                            ? '세부 업무 완료 · 총괄 담당자의 최종 확인이 필요합니다.'
                            : group.confirmations.length
                              ? `세부 실행 업무 완료 · 중간 상위 업무 ${group.confirmations.length}건의 완료 확인이 남아 있습니다.`
                              : '완료 연결 정보를 확인한 뒤 상위 업무를 완료해 주세요.'}
                        </p>
                        {group.readyToConfirm && (
                          <Button
                            type="button"
                    ...
                    {children.length > 0 && (
                      <Collapsible
                        open={isExpanded}
                        onOpenChange={(open) => {
                          if (!autoExpanded) toggle(item.id, open);
                        }}
                      >
                        <CollapsibleTrigger
                          disabled={autoExpanded}
                          aria-label={`${item.title} 세부 업무 ${autoExpanded ? '검색 중 펼침' : isExpanded ? '접기' : '펼치기'}`}
                        ...
                        <CollapsibleContent>
                          ...
                          >
                            {children.map(({ item: child, depth, matched }) => (
                              <li
                                key={child.id}
                              ...
                              >
                                  ...
                                  <Select
                                    ...
                                    onValueChange={(value) => {
                                      if (
                                        ...
                                        onEdit(child, value as FarmWorkStatus);
                                    }}
                                  >
                                    <SelectTrigger
                                    ...
                                    <SelectContent>
                                      {FARM_WORK_STATUSES.map((value) => (
                                        <SelectItem key={value} value={value}>
                                          {FARM_WORK_STATUS_LABELS[value]}
                                        </SelectItem>
                                      ))}
                                    </SelectContent>
                                  </Select>
                                ...
                                {!canEdit(child) && (
                                  <p className="text-sm text-slate-600">
                                    완료된 사업 또는 상위 업무의 상태 변경은
                                    잠겨 있습니다. 처리 내용은 빠른 수정에서
                                    남길 수 있습니다.
                                  </p>
                                )}
                                {showHistory.has(item.id) &&
