...
통합 현황과 프로젝트 관리 상단은 같은 사업연도 상태를 공유합니다. 연도 선택은 사업 등록연도 기준입니다. 상태 수치·업무·구독 등은 현재 저장 데이터로 계산하므로 과거 연말 스냅샷과 다릅니다. 검색·상태·위험 조건은 하단 목록만 좁힙니다.
...
통합 현황의 최근 이력과 확인 필요도 선택 사업연도를 따릅니다. 농가 공통 데이터 문제는 참여 농가 기준, 설치·구독·참여 중복 문제는 선택 프로젝트 기준입니다. 확인 필요를 눌러 다른 화면으로 이동할 때 목적지 필터를 자동 설정하지는 않습니다.
