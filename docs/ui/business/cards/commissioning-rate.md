...
선택한 사업 참여 정보 중 시운전 완료 날짜가 입력된 비율을 확인합니다.

## 표시값·계산식

완료 건수는 `commissioningDate`가 비어 있지 않은 사업 참여 레코드 수(`businessCommissioning`)입니다. 비율은 `Math.round(완료 건수 / businessRecords.length × 100)`이며 정수 %로 표시합니다. 보조값은 완료 건수/전체 참여 건수입니다. 분모가 0이면 `-`입니다.
