...
프로젝트 참여 농가 중 시운전 완료일이 입력된 비중을 확인합니다.

## 표시값·계산식

해당 참여 레코드의 `commissioningDate`가 비어 있지 않은 건수/전체 참여 수를 정수 반올림합니다. 카드에는 비율, 완료/참여 수, 날짜 입력 기준이라는 설명, 비율 막대가 표시됩니다. 분모가 0이면 `-`이며 막대는 0입니다.
