...
2026-09-11 배치 변경: 진행 요약은 **프로젝트 진행률·참여 농가·확인 필요 → 확인 항목 → 농가 진행상황 → 접힌 사업 진행률 산정 근거** 순으로 표시한다. 설치·시운전·교육 비율·완료/미완료 개소·농가별 아래 펼침은 농가 진행상황 한곳에서 보존한다. 기존 6개 KPI와 별도 설치·운영 완료율의 중복 표시는 정리했고, 구독·서류·정산은 해당 상세 탭에 유지한다. 확인 항목에는 구독·정산 위험 탭 바로가기도 포함한다. 아래 카드 목차와 이전 배치 설명은 계산·기능 참조용이다.
...
2026-09-07 개편: 진행 요약 / 농가·구독 / 하위 업무 / 제출서류 / 정산 / 처리 이력 탭으로 구분한다. 단계별 설치·운영 지표는 요약에서 펼쳐 본다. 이전 상세의 탭·스크롤을 복원하며 농가로 진입할 때 사업 문맥을 유지한다. 완료 사업의 서류는 내용·링크 조회가 가능하고 수정만 제한한다. [개편 기준](../ux-redesign.md).
